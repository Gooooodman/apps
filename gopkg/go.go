...
var spool = filepath.Join(os.TempDir(), "gopkg")
...
with the dependencies listed in file Gopkgs.

Any go tool command can run this way, but "gopkg go get"
...
func runGo(cmd *Command, args []string) {
	...
	if len(args) > 0 && args[0] == "get" {
		...
		fmt.Fprintln(os.Stderr, "Use 'gopkg go install' instead.")
		fmt.Fprintln(os.Stderr, "Run 'gopkg help go' for usage.")
		os.Exit(2)
	}
...
func prepareGopath() (gopath string) {
	...
	if isDir {
		return filepath.Join(dir, "Gopkgs", "_workspace")
	}
	log.Println(strings.TrimSpace(noSourceCodeWarning))
	g, err := ReadAndLoadGopkgs(filepath.Join(dir, "Gopkgs"))
	if err != nil {
		log.Fatalln(err)
	}
...
// findGopkgs looks for a directory entry "Gopkgs" in the
// current directory or any parent, and returns the containing
// directory and whether the entry itself is a directory.
// If Gopkgs can't be found, findGopkgs returns "".
// For any other error, it exits the program.
func findGopkgs() (dir string, isDir bool) {
	wd, err := os.Getwd()
	if err != nil {
		log.Fatalln(err)
	}
	return findInParents(wd, "Gopkgs")
}
...
warning: outdated Gopkgs missing source code
